...
class Emojifier {
    ...
    private static void whichEmoji(Face face){
        ...
        if (isSmiling){
            if (isLeftEyeClosed && isRightEyeClosed)
                emoji = Emoji.SMILE_BOTH_CLOSE_EYES;
            else if(isLeftEyeClosed && !isRightEyeClosed)
                emoji = Emoji.SMILE_RIGHT_OPEN_EYE;
            else if(!isLeftEyeClosed && isRightEyeClosed)
                emoji = Emoji.SMILE_LEFT_OPEN_EYE;
            else
                emoji = Emoji.SMILE_BOTH_OPEN_EYES;

        } else {
            if (isLeftEyeClosed && isRightEyeClosed)
                emoji = Emoji.FROWNING_BOTH_CLOSE_EYES;
            else if(isLeftEyeClosed && !isRightEyeClosed)
                emoji = Emoji.FROWNING_RIGHT_OPEN_EYE;
            else if(!isLeftEyeClosed && isRightEyeClosed)
                emoji = Emoji.FROWNING_LEFT_OPEN_EYE;
            else
    ...
    enum Emoji{
        SMILE_BOTH_OPEN_EYES,
        SMILE_BOTH_CLOSE_EYES,
        SMILE_LEFT_OPEN_EYE,
        SMILE_RIGHT_OPEN_EYE,
        FROWNING_BOTH_OPEN_EYES,
        FROWNING_BOTH_CLOSE_EYES,
        FROWNING_LEFT_OPEN_EYE,
        FROWNING_RIGHT_OPEN_EYE,
    }
}
